...
- 一覧の絞り込み対象には含めなくてよい
- 詳細画面では表示可
- 将来、通知や強調表示に使えるように残しておく

---

## Q54. `session_token` の生成方式（形式・長さ）を教えてください。

**A:** Python の `secrets.token_urlsafe(32)` を採用する。

仕様決定:
- 生成方式: `secrets.token_urlsafe(32)`
- 保存値: URL-safe なランダム文字列
- 目安長: おおむね 43 文字前後
- DB 上は `TEXT UNIQUE`
- UUID v4 ではなく、十分ランダムなトークン文字列を使う
- Bearer Token 用なので可読性よりランダム性優先
...
仕様決定:
- 保存例: `/uploads/photos/2026/03/uuid-01.jpg`
- DB の `photo_path` には相対パスを保存する
- フル URL は保存しない
- クライアントは API ベース URL または同一公開 URL に連結して表示する

理由: ドメイン変更に強く、本番/開発切り替えがしやすい。

---

## Q56. `sync_queue.payload` の JSON スキーマを教えてください。

**A:** operation ごとに固定スキーマを採用する。
...
```
- `base_revision` は不要
- `user_id` / `created_by` / `updated_by` / `deleted_*` / `revision` はクライアントから送らない（API 側で設定）
- `sync_state` はクライアント専用属性のため payload に含めない
...
- `base_revision` 必須
- `fields` は変更項目のみ
- API 側で revision check を行い、成功時に `revision = revision + 1`
...
```
- 物理削除ではない。API 側で tombstone 更新（`deleted_flag=1`, `deleted_at`, `deleted_by`, `revision+1`）
- API 側で `server_updated_at=now` も更新する

### payload に含めてはいけない項目
`user_id` / `created_by` / `updated_by` / `deleted_*` / `server_updated_at` / `revision`（update/delete では `base_revision` のみ可）/ `sync_state`
→ `sync_state` はクライアント専用、その他はサーバーの責務

---

## Q57. `work_logs.sync_state` はサーバー側にも保持しますか？

**A:** 保持しない。`sync_state` はクライアント専用属性とする。

### 仕様決定
- サーバー側 `work_logs` から `sync_state` を削除する
- サーバー側 `work_logs` から `local_updated_at` も削除する
- クライアント側 `WorkLogLocal` は `local_updated_at` / `sync_state` を保持する

### サーバー側 `work_logs` が保持する項目
`log_uuid` / `user_id` / `equipment_id` / `record_type` / `status` / `title` / `symptom` / `work_detail` / `result` / `priority` / `recorded_at` / `needs_followup` / `followup_due` / `server_updated_at` / `revision` / `created_by` / `updated_by` / `deleted_flag` / `deleted_at` / `deleted_by`

### 補足
- `local_only` / `dirty` / `failed` / `conflict` は端末ごとに異なるため、サーバーの単一レコード属性にはしない
- サーバーは事実だけを持ち、同期状態は `revision` / `server_updated_at` / `deleted_flag` で表現する

---

## Q58. オフライン新規作成時の `WorkLogLocal.revision` の初期値は何にしますか？

**A:** `0` にする。`NULL` にはしない。

### 仕様決定
- オフライン新規作成直後の `WorkLogLocal.revision = 0`
- 同時に `server_updated_at = NULL`、`sync_state = local_only` とする
- `create` 成功後はサーバー返却値で `revision = 1` 以上に更新する
- `create` 成功後は `server_updated_at = サーバー時刻`、`sync_state = synced` に更新する

### ルール整理
- `revision = 0`: ローカル新規、未送信
- `revision >= 1`: サーバーに存在する正式レコード
- `revision = 0` のレコードに対して `update` / `delete` を直接送らず、先に `create` を送る

### 理由
`NULL` を使うより、`0` を「未採番のローカル新規」の予約値にした方が条件分岐が単純になる

---

## Q59. `sync_pull` の `items` は `WorkLogLocal` と同一型にしますか？

**A:** 同一型にしない。別 DTO に分ける。

### 仕様決定
- `sync_pull` の `items` はサーバー返却用 DTO として `WorkLogDTO[]` を返す
- `WorkLogDTO` には `local_updated_at` / `sync_state` を含めない
- クライアントは `WorkLogDTO` を `WorkLogLocal` に変換して IndexedDB へ保存する

### `WorkLogDTO` が保持する項目
`log_uuid` / `user_id` / `equipment_id` / `record_type` / `status` / `title` / `symptom` / `work_detail` / `result` / `priority` / `recorded_at` / `needs_followup` / `followup_due` / `server_updated_at` / `revision` / `created_by` / `updated_by` / `deleted_flag` / `deleted_at` / `deleted_by`

### `WorkLogDTO` が持たない項目
`local_updated_at` / `sync_state`

### 変換ルール
- 既存 `WorkLogLocal.local_updated_at` は保持する
- 新規取り込み時の `local_updated_at` は `NULL` でもよいが、MVP では `server_updated_at` を入れてよい
- `sync_state` は通常 `synced` とする
- 既存レコードが `conflict` の場合は `conflict` を維持する

### 競合時
- `sync_push` で `409 Conflict` を返す
- `server_entity` は `WorkLogDTO` とする
- クライアントはローカルの `WorkLogLocal.sync_state = conflict` として保持する

---

## TEST_SPEC.md レビュー時の不明点・確認事項

## Q60. 禁止フィールドを送った場合は「エラー」か「無視」か、どちらで統一しますか？

**A:** エラーにする。無視しない。

### 仕様決定
- 禁止フィールドが request payload に含まれていたら `HTTP 400 Bad Request` を返す
- 少なくとも `user_id` / `created_by` / `updated_by` / `deleted_by` / `server_updated_at` / `revision` / `sync_state` はクライアント送信禁止とする
- `update` / `delete` の `base_revision` は許可する
- `create` に `base_revision` を送った場合は `HTTP 400`
- `delete` に `fields` を送った場合は `HTTP 400`
- `update` の `fields` 内に禁止項目が含まれていたら `HTTP 400`

### 理由
API 側で管理する値を黙って無視すると、不正 payload を通してしまい、実装バグや不正入力を見逃しやすい。禁止入力は明示的に失敗させる。

---

## Q61. `include_deleted=true` を User が指定した場合の挙動は `403` と「通常一覧で返す」のどちらですか？

**A:** `403 Forbidden` にする。

### 仕様決定
- Admin は `include_deleted=true` を使用できる
- User が `include_deleted=true` を指定したら `HTTP 403 Forbidden`
- User の通常一覧は常に `deleted_flag=0` のみ返す

### 理由
削除済み参照は Admin 専用機能なので、User が要求した時点で権限外操作として拒否するのが自然。

---

## Q62. XSS 対策は API レスポンスでエスケープしますか？ それとも UI 描画時にエスケープしますか？

**A:** 表示時のエスケープはクライアント側必須、サーバーは原則プレーンテキストとして返す。

### 仕様決定
- サーバーは入力文字列を原則そのまま保存する
- API は JSON の文字列として返し、HTML として解釈させる前提の加工はしない
- クライアントは `innerHTML` を使わず、テキストとして描画する
- 必要に応じて表示時に HTML エスケープする
- Markdown やリッチテキストは MVP では扱わない

### 補足
- 危険な制御文字や過剰サイズはバリデーションで弾いてよい
- `<script>alert(1)</script>` を保存できても、画面ではただの文字列として表示され、実行されないことを保証する

---

## Q63. `Origin` ヘッダーがない CORS リクエストは許可しますか？

**A:** 拒否する。`HTTP 403 Forbidden` とする。

### 仕様決定
- `Origin` ヘッダーが存在しない場合は `HTTP 403 Forbidden`
- 例外運用は MVP では設けない
- 許可 Origin は `https://garyohosu.github.io` / `http://localhost` / `http://127.0.0.1` の 3 つに固定する

### 理由
この API は GitHub Pages の PWA から使う前提なので、Origin なしを通す理由がない。ここを曖昧にすると CORS とアクセス制御の期待が崩れる。
