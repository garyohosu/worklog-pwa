...
```
- `base_revision` は不要
- `user_id` / `created_by` / `updated_by` / `deleted_*` / `revision` / `sync_state` はクライアントから送らない（API 側で設定）
...
### payload に含めてはいけない項目
`user_id` / `created_by` / `updated_by` / `deleted_by` / `server_updated_at` / `revision`（update/delete では `base_revision` のみ可）/ `sync_state`
→ これらはサーバーの責務
