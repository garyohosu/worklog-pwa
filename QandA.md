...
SPEC §4.2 には「全記録参照」とありますが、作業記録の作成・編集・論理削除については明記がありません。

選択肢:
- A. 参照のみ（作成・編集・削除は自分が所有するものだけ）
- B. 全記録の参照・編集・削除が可能（作成は自分名義で可）
- C. Admin は作業記録の作成自体を行わない（管理専任）

---

## Q43. Admin は QR 読取・「設備なし」選択を実行できますか？

Admin が現場に出て作業記録を作る想定があるかどうかに依存します。
Q42 で Admin に作業記録の作成を許可する場合、設備選択や QR 読取も必要になります。

---

## Q44. Admin もオフライン下書き・同期機能を使いますか？

Admin が現場で作業記録を作る想定なら同期も必要です。
Admin は管理操作（ユーザー管理・集計確認）はオンライン前提でよいですが、作業記録を作る場合はオフライン機能が必要になります。

---

## Q45. トップページ（index.html）と「利用案内ページ」の内容を教えてください。

Q41 の AdSense 表示対象に「トップページ」「利用案内ページ」が登場しますが、内容が未定義です。

確認事項:
1. `index.html`（トップページ）はサービス紹介ページですか？それともログイン済みなら自動リダイレクト、未認証なら案内表示するページですか？
2. 「利用案内ページ」は `/about/` や `/guide/` のような独立したページですか？それとも `index.html` 内の一部ですか？
3. これらのページは MVP に含めますか？
