...
- User が `include_deleted=true` を指定した場合は `HTTP 403 Forbidden`
...
### 禁止フィールド
- `user_id` / `created_by` / `updated_by` / `deleted_by` / `server_updated_at` / `revision` / `sync_state` はクライアント送信禁止とする
- 禁止フィールドが request payload に含まれていた場合は `HTTP 400 Bad Request`
- `create` に `base_revision` を含めた場合は `HTTP 400`
- `delete` に `fields` を含めた場合は `HTTP 400`
- `update` の `fields` 内に禁止項目が含まれていた場合は `HTTP 400`
...
- `Origin` ヘッダーが存在しないリクエストは `HTTP 403 Forbidden`
- 管理機能は role チェック必須
- 入力値バリデーション実施
- SQL 注入対策実施
...
- API は JSON の文字列として返し、HTML 解釈前提のエスケープ済み文字列へ書き換えない
- クライアントは `innerHTML` を使わず、テキストとして描画する
- `<script>` 等を含む文字列も、画面上ではただの文字列として表示され、実行されないことを保証する
